...
>(function TopologyCanvas(
  ...
  const fitToContent = useCallback(() => {
    const rect = svgRef.current?.getBoundingClientRect();
    if (!rect || (topology.devices.length === 0 && !topology.blocks?.length))
      return;
    const xs = [
      ...topology.devices.map((d) => d.position.x),
      ...(topology.blocks ?? []).map((b) => b.x),
    ];
    const ys = [
      ...topology.devices.map((d) => d.position.y),
      ...(topology.blocks ?? []).map((b) => b.y),
    ];
    const xs2 = [
      ...topology.devices.map((d) => d.position.x),
      ...(topology.blocks ?? []).map((b) => b.x + b.width),
    ];
    const ys2 = [
      ...topology.devices.map((d) => d.position.y),
      ...(topology.blocks ?? []).map((b) => b.y + b.height),
    ];
  ...
  const handleDevicePointerDown = useCallback(
    (e: React.PointerEvent, deviceId: string) => {
      ...
      const group =
        selectedDeviceIds.length > 1 && selectedDeviceIds.includes(deviceId)
          ? topology.devices
    ...
    (e: React.PointerEvent) => {
      ...
      if (selectMode || e.ctrlKey || e.metaKey) {
        ...
        const apply = (ev: PointerEvent) => {
          ...
          if (
            !sel.moved &&
            Math.hypot(ev.clientX - e.clientX, ev.clientY - e.clientY) < 4
          )
            return;
          sel.moved = true;
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent, deviceId: string | null) => {
      e.preventDefault();
  ...
  return (
    <div className="relative flex-1 bg-[#0A0E1A] overflow-hidden">
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse at center, transparent 50%, rgba(2,9,20,0.4) 100%)',
        }}
      />
      ...
      {connectMode && (
        <div className="absolute top-3 left-1/2 -translate-x-1/2 flex items-center gap-2 rounded-lg bg-[#1C2538]/90 border border-[--color-accent-cyan]/40 px-4 py-2 text-xs text-[--color-accent-cyan] shadow-lg animate-fade-in-up">
          ...
          <span className="font-medium">
            {connectingFromId
              ? 'Clique no segundo equipamento para conectar'
              : 'Clique no primeiro equipamento'}
          </span>
        </div>
      )}
      ...
      <div className="absolute bottom-3 left-3 px-3 py-1.5 rounded-lg bg-[#111A2C]/80 border border-[--color-border-primary]/60 text-[10px] font-mono text-[--color-text-muted] glass">
        {Math.round(viewport.zoom * 100)}% · {topology.devices.length} disp. ·{' '}
        {topology.connections.length} conexões
      </div>
      ...
      <div className="absolute bottom-3 right-3 flex items-center gap-1">
        ...
        <button
          ...
          title="Relatório da rede em JSON para conferência"
          className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-[#111A2C]/80 border border-[--color-border-primary]/60 text-[10px] font-medium text-[--color-text-muted] hover:text-[--color-text-primary] hover:border-[--color-accent-cyan]/40 transition-colors cursor-pointer glass"
        >
          <FileJson2 size={11} />
          Relatório
        ...
        <button
          onClick={() => setSessionReportOpen(true)}
          title="Relatório de sessão (comandos executados no console)"
          className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-[#111A2C]/80 border border-[--color-border-primary]/60 text-[10px] font-medium text-[--color-text-muted] hover:text-[--color-text-primary] hover:border-[--color-accent-green]/40 transition-colors cursor-pointer glass"
        >
          <ScrollText size={11} />
          Sessão
        </button>
      ...
      {/* Relatório da rede */}
      <NetworkReport open={reportOpen} onClose={() => setReportOpen(false)} />
